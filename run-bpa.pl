...
###############################################################################
#                               run-bpa.pl
#       		Wrapper script for BPA-2.1.0 pipeline
#                               February 4, 2014
#                               NCGR: www.ncgr.org
###############################################################################
...
Unitigs:
	...
	Usage: $0 -m unitigs <-i "fastq(s)"> <-c> <-k> [-v]
...
OLC:
	...
	Usage: $0 -m OLC <-i "fasta(s)"> [-t threads] [-u "cap3 options"] [-e <= 1.0]
...
Scaffold:
	...
	Usage: $0 -m scaffold <-r "read file(s)"> <-R reference.fa> <-k kmer> [-i maximum mismatch bwa] [-p min pairs] [-o output prefix] [-s seed length] [-t threads]
...
Gapclose:
	...
	Usage: $0 -m gapclose <-f forward reads> <-r reverse reads> <-R fasta file to gap close> <-l maximum read length> [-i insert size] [-t threads]
...
BLAST:
	...
	Usage: $0 -m blast <-i input file> <-R blast formatted reference> [-o output filename] [-t threads] [-s shell] [-e e-value] [-f out format] [-g genetic code]
...
ESTscan:
	...
	Usage: $0 -m estscan <-i input file> <-s scoring matrix> [-o output prefix] [-l minimum result length]
...
HMMscan:
	...
	Usage: $0 -m hmmscan <-i input file> <-d HMMER3 database> [-o output prefix] [-t threads]
